...
def get_recent_cars():
    return __get_recent_cars_from_db__()
...
def get_latest_car():
    max_serial = db.session.query(func.max(Car.serial_number)).first()
    return Car.query.filter_by(serial_number=max_serial).first()
...
@cache.memoize()
def __get_recent_cars_from_db__():

    start_date = datetime.utcnow() + timedelta(days=-60)
    return Car.query.order_by(Car.created_date.desc()).filter(Car.created_date >= start_date).all()
